...
app.post('/save-configuration', ensureAuthenticated, (req, res) => {
  ...
  db.run('INSERT INTO configurations (username, siteName, cameraCount, retentionDays, totalStorage) VALUES (?, ?, ?, ?, ?)',
    [req.session.user.username, siteName, cameraCount, retentionDays, totalStorage], (err) => {
      if (err) return res.status(500).send('Error saving configuration.');
...
app.get('/get-configurations', ensureAuthenticated, (req, res) => {
  db.all('SELECT id, siteName, cameraCount, retentionDays, totalStorage, timestamp FROM configurations WHERE username = ?', 
    [req.session.user.username], (err, rows) => {
      if (err) return res.status(500).send('Error retrieving configurations.');
      res.json(rows);
  });
});
...
app.get('/load-configuration', ensureAuthenticated, (req, res) => {
  ...
  db.get('SELECT * FROM configurations WHERE id = ? AND username = ?', 
    [configId, req.session.user.username], (err, row) => {
      if (err || !row) return res.status(404).send('Configuration not found.');
      res.json(row);
  });
});
...
app.delete('/delete-configuration', ensureAuthenticated, (req, res) => {
  const configId = req.query.id;
  db.run('DELETE FROM configurations WHERE id = ? AND username = ?', 
    [configId, req.session.user.username], (err) => {
      if (err) return res.status(500).send('Error deleting configuration.');
      res.send('Configuration deleted.');
  });
});
